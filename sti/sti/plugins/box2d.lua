...
return {
	...
	box2d_init = function(map, world)
		...
		local function addObjectToWorld(objshape, vertices, userdata, object, commonbody)
			local shape
			local currentBody = body

			if userdata.properties.dynamic == true then
        local x,y = userdata.object.x + userdata.object.w / 2, userdata.object.y + userdata.object.h / 2
				currentBody = love.physics.newBody(world, map.offsetx + x, map.offsety + y, 'dynamic')
        for i = 1,#vertices,2 do
          vertices[i] = vertices[i] - x
          vertices[i+1] = vertices[i+1] - y
        end
      else
        currentbody = commonbody
			end
      ...
      if _G[userdata.properties.module] then
        obj.module = _G[userdata.properties.module](obj)
        fixture:setUserData(obj.module)
      end
      ...
      return currentBody
        ...
        local currentbody
				...
				for _, triangle in ipairs(triangles) do
					currentbody = addObjectToWorld(o.shape, triangle, userdata, tile or object, currentbody)
				end
	...
	-- @param sx Scale on X
	-- @param sy Scale on Y
	box2d_draw = function(map, tx, ty, sx, sy, mask)
		local collision = map.box2d_collision
      ...
      if not mask or (bitand(select(1,obj.fixture:getFilterData()), mask or 0) > 0) then
        ...
        elseif shape_type == "circle" then
          ...
          love.graphics.circle("line", x,y,obj.shape:getRadius())
        else
  ...
  box2d_add = function(map, object)
    table.insert(map.box2d_collision, object)
  end,
  
  box2d_foreach = function(map, group, mask, func)
    local collision = map.box2d_collision
		for _, obj in ipairs(collision) do
      if not obj.fixture:isDestroyed() then
        local c,m,g = obj.fixture:getFilterData()
        local group = group or 0
        if group == 0 or (group > 0 and group == g) or (group < 0 and group ~= g) then
          if not mask or bitand(c, mask) > 0 then
            func(obj)
          end
        end
      end
  ...
  box2d_remove = function(map, fixture, newgid)
    local collision = map.box2d_collision
		for i, obj in ipairs(collision) do
      if obj.fixture == fixture then
        local tile = obj.object
        if tile then
          map:setLayerTile(tile.layer.name, 1+tile.x/16, 1+tile.y/16, newgid or 0)
          if obj.object.gid then
            obj.object.gid = newgid or 0
          end
        end
        obj.fixture:destroy()
        table.remove(collision,i)
